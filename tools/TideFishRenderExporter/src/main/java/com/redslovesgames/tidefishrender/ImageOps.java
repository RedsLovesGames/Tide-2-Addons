...
final class ImageOps {
    ...
    static Bounds alphaBounds(NativeImage img){int minX=img.getWidth(),minY=img.getHeight(),maxX=-1,maxY=-1;for(int y=0;y<img.getHeight();y++)for(int x=0;x<img.getWidth();x++){int a=(img.getColorAbgr(x,y)>>>24)&255;if(a>0){if(x<minX)minX=x;if(y<minY)minY=y;if(x>maxX)maxX=x;if(y>maxY)maxY=y;}}return maxX<0?null:new Bounds(minX,minY,maxX,maxY);}
    static NativeImage cropWithPadding(NativeImage src,Bounds b,int pad){NativeImage out=new NativeImage(NativeImage.Format.RGBA,b.width()+pad*2,b.height()+pad*2,true);for(int y=0;y<b.height();y++)for(int x=0;x<b.width();x++)out.setColorAbgr(x+pad,y+pad,src.getColorAbgr(b.minX()+x,b.minY()+y));return out;}
