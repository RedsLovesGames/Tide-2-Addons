(()=>{
  ...
  const load=()=>index?Promise.resolve(index):(loading||(loading=fetch('assets/fish-search-index.json').then(r=>r.ok?r.json():[]).then(v=>index=v).catch(err=>{console.warn('Fish Wiki search index unavailable',err);return index=[]})));
  function escapeHtml(s){return String(s??'').replace(/[&<>\"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;'}[c]))}
  const fishHash=id=>encodeURIComponent(String(id||'').replace(':','__'));
  async function augment(){const q=input.value.trim().toLowerCase();if(q.length<2)return;const fish=await load();if(input.value.trim().toLowerCase()!==q)return;const hits=fish.filter(f=>[f.name,f.id,f.mod,f.group,f.rarity,f.location].join(' ').toLowerCase().includes(q)).slice(0,8);if(!hits.length)return;const block=document.createElement('div');block.className='fish-search-group';block.innerHTML=`<div style="padding:10px 14px 5px;color:var(--muted);font:10px ui-monospace,monospace;text-transform:uppercase;letter-spacing:.1em">Fish Wiki</div>${hits.map(f=>`<a href="fish/#${fishHash(f.id)}" style="display:block;padding:10px 14px;text-decoration:none;border-top:1px solid var(--line)"><strong>${escapeHtml(f.name)}</strong><small style="display:block;color:var(--muted);margin-top:2px">Fish Wiki · ${escapeHtml(f.mod)} · ${escapeHtml(f.location||f.group)}</small></a>`).join('')}`;results.append(block)}
